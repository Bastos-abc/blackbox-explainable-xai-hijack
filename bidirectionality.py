...
class Orchestrator:
    ...
    def compute_edge_features_daily_sampling(self, ts: str=None, timespan: int=30, override: bool=False, 
        ...
        if 'bidi' in feat:
            ...
            # Bidi New Edges
            #df = bidirectional_links(positive_ne_links, rib_file, bgp_topo_files, irr_topo_files,
            #                         threshold_days_appearance=1)
            #df_ne_positive = df_ne_positive.merge(df, how='left')

            #df = bidirectional_links(negative_ne_links, rib_file, bgp_topo_files, irr_topo_files,
            #                        threshold_days_appearance=1)
            #df_ne_negative = df_ne_negative.merge(df, how='left')
        ...
        if 'nb_vps' in feat:
            ...
            #Sample New Edges
            #df = self.cnvp.count_neighboring_vps(positive_ne_links)
            #df_ne_positive = df_ne_positive.merge(df, how='left')
            #df = self.cnvp.count_neighboring_vps(negative_ne_links)
            #df_ne_negative = df_ne_negative.merge(df, how='left')
        ...
        #df_ne_positive.to_csv(outfile_ne_positive, sep=' ', index=False)
        #df_ne_negative.to_csv(outfile_ne_negative, sep=' ', index=False)

    def compute_edge_features_links(self, ts: str, outfile, links=set, timespan: int=30, return_df: bool=False, feat=["bidi", "nb_vps"]):
        date = datetime.strptime(ts, "%Y-%m-%d")
...
def launch_orchestrator(\
    ...
    link_file=None,
    method="clusters",
    outfile=None, return_df=False, feat=["bidi", "nb_vps"]):
    """Compute bidirectionality features and store them in the database."""

    if daily_sampling:
        o = Orchestrator(method, db_dir=db_dir)
        o.compute_edge_features_daily_sampling(date, override=override, timespan=30, feat=feat)
    else:
        links = set()
        ...
        o = Orchestrator(method, db_dir=db_dir)
        if return_df:
            return o.compute_edge_features_links(date, outfile, links=links, timespan=30, return_df=return_df, feat=feat)
        else:
            o.compute_edge_features_links(date, outfile, links=links, timespan=30, return_df=return_df, feat=feat)
