...
class TopoGenerator:
    ...
    def get_topo_date(paramslist):
        ...
        for cur_date in TopoGenerator.daterange(first_day, last_day):
            ...
            # Get the RIB of the peers from which the updates were collected.
            rib_filename = db_dir+'/topology/'+cur_date.strftime("%Y-%m-%d")+"_ribs2.txt"
            ...
            # Get the RIB from the CAIDA data.
...
def generate_topology(\
    ...
    if date_end is None:
        ...
        paramslist = [db_dir, date, override, nbdays, 'merged_topology']
        tc.get_topo_date(paramslist)
    else:
