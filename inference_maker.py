...
all_feats_cat = {
    ...
    "aspath": ["as1", "as2", "degree", "cone", "cone_degree"],
...
class InferenceMaker:
    ...
    def make_inference(self, df):
        ...
        for fpr_weight in self.fpr_weights:
            j += 1
            print('Inference weight',j , 'of', len(self.fpr_weights))
    ...
    #        df = pd.DataFrame(self.results, columns=['as1', 'as2', 'asp', 'label', 'proba','sensitivity'])
    #    except:
    #        print('Fail to convert to pandas in inference_maker.py')
    #        return None
    #    return df
...
@click.option("--nb_days_training_data", default=30, help="Number of day for which to take the sampled links", type=int)
@click.option("--outfile", default=None, help="File to output the results", type=str)
'''

def run_inference_maker(date, db_dir="db", features="aspath,bidirectionality,peeringdb,topological", overide=0,
                        input_list=None, input_file=None, paper_eval=0, fpr_weights='1', method="clusters",
                        nb_days_training_data=30, outfile=None, return_df=False):
    ...
    if return_df:
        ...
        return InfMake.to_df()
    elif outfile != None:
        fout = open(outfile,'w')
        print(InfMake.to_string(), file=fout)
        fout.close()
        del df
        del InfMake
    else:
