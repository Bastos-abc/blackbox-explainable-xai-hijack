...
froot = '/home/bastos/dfoh_exec/'
faspath = froot + 'aspathfeat/main/'
fbidi = froot + 'bidirectionality/main/'
fdb = froot + 'db/main/'
frunner = froot + 'runner-main/main/'
fnewedge = froot + 'newedge/main/'
fpeering = froot + 'peeringdb/main/'
fsampling = froot + 'sampling/main/'
ftopofeat = froot + 'topofeat/main/'
finference = froot + 'inference/main/'
...
def command_to_csv(args, debug=False):
    tmp_file = 'command_to_csv_runner.tmp'
    path = os.getcwd()
    if len(args) > 2:
        if args[1].startswith('aspath'):
            folder = faspath
        elif args[1].startswith('topo'):
            folder = ftopofeat
        elif args[1].startswith('orchestrator') and args[2].startswith('--da'):
            folder = fpeering
        elif args[1].startswith('orchestrator'):
            folder = fbidi
        elif args[1].startswith('inference'):
            folder = finference
        else:
            folder = path
    os.chdir(folder)
    cmd = ''
    args.append('--outfile ' + tmp_file)
    #for c in args
    #    cmd += ' ' + c


    print('Utils Broker -> path',folder)
    print(args)
    if debug:
        subprocess.run(args)
        os.chdir(path)
