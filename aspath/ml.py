...
import aspath.prepare_dataset as prepd
import pandas as pd
...
def build_model_for_day(db_dir, date, metric, method, nbdays, no_save=True):
    train_dataset = '{}/aspath_models_trustee'.format(db_dir)
    ...
    X = prepd.get_dataset_for_one_day(db_dir, date, method, metrics=metric.split("_"))
    if X is None:
    ...
    for d in all_dates:
        ...
        tmp_df = prepd.get_dataset_for_one_day(db_dir, d, method, metrics=metric.split("_"))
        if tmp_df is None:
    ...
    if X is None or len(X.index) == 0:
        ...
        return None
    feat_file = '{}/{}_{}.csv'.format(train_dataset, date, metric)

    X.to_csv(feat_file, sep=' ', index=False)
