...
class Sampling:
    def __init__(self, date, db_dir, overide, method, k_neg, k_pos, output):
        self.date = date
        self.topo = nx.Graph()
        ...
        try:
            ut.create_directory("{}/sampling/".format(self.output))
            ut.create_directory("{}/sampling_cluster/".format(self.output))
            ut.create_directory("{}/sampling/negative".format(self.output))
            ut.create_directory("{}/sampling/positive".format(self.output))
            ut.create_directory("{}/sampling/negative/sampling_aspath".format(self.output))
            ut.create_directory("{}/sampling/negative/sampling".format(self.output))
            ut.create_directory("{}/sampling/positive/sampling_aspath_{}".format(self.output, self.method))
            ut.create_directory("{}/sampling/positive/sampling_{}".format(self.output, self.method))
        ...
        if os.path.exists(fn_topo):
            ...
            for as1, as2 in suspicious_edges:
                try:
                    ...
                    print('{} and {} were not in the graph'.format(as1, as2))
    ...
    def build_negative_sampling(self, size):
        ...
        fn_sampling = "{}/sampling/negative/sampling/{}_negative.txt".format(self.output, self.date)
    ...
    def build_positive_sampling(self, size, thresholds):
        fn_aspaths = "{}/paths/{}-{}-01_paths.txt".format(self.db_dir, self.date.split("-")[0], self.date.split("-")[1])
        fn_sampling = "{}/sampling/positive/sampling_{}/{}_positive.txt".format(self.output, self.method, self.date)
    ...
    def build_negative_sampling_aspath(self, size):
        fn_aspaths = "{}/paths/{}-{}-01_paths.txt".format(self.db_dir, self.date.split("-")[0], self.date.split("-")[1])
        fn_sampling = "{}/sampling/negative/sampling_aspath/{}_negative.txt".format(self.output, self.date)
    ...
    def build_positive_sampling_aspath(self, size, thresholds):
        fn_aspaths = "{}/paths/{}-{}-01_paths.txt".format(self.db_dir, self.date.split("-")[0], self.date.split("-")[1])
        fn_sampling = "{}/sampling/positive/sampling_aspath_{}/{}_positive.txt".format(self.output, self.method, self.date)
...
def sampler_aux(date, db_dir, overide, nb_threads, size, thres, method, k_neg, k_pos, output):
    sampler = Sampling(date, db_dir, overide, method, k_neg, k_pos, output)
...
@click.option("--thresholds", default="0,10,50,100,500,1000,1500,3000,5000,100000", help="Positive sampling thresholds (format thres1,thres2,thres3,thres4)", type=str)
...
@click.option("--nb_threads", default=10, help="Number of threads to build sampling with", type=int)
...
@click.option("--output", default="/tmp", help="Directory to save samplings", type=str)


def run_orchestrator(date, size, thresholds, db_dir, overide, nb_threads, end_date, method, k_neg, k_pos, output):
    k_neg = float(k_neg)
    k_pos = float(k_pos)
    ...
    if end_date is not None:
        ...
        with ProcessPoolExecutor(max_workers=nb_threads) as exec:
            for d in all_dates:
                all_procs.append(exec.submit(sampler_aux, d, db_dir, overide, 1, size, thres, method, k_neg, k_pos, output))
    ...
    sampler = Sampling(date, db_dir, overide, method, k_pos, k_neg, output)
